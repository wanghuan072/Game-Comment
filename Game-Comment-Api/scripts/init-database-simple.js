...
async function initializeDatabase() {
  try {
    ...
    const createGamesTable = `
      CREATE TABLE IF NOT EXISTS ${PROJECT_PREFIX}_games (
        id SERIAL PRIMARY KEY,
    ...
    `;
    await sql.unsafe(createGamesTable);

    // 3. 创建项目特定评论表（带前缀）
    console.log(`💬 创建评论表 ${PROJECT_PREFIX}_comments...`);
    const createCommentsTable = `
      CREATE TABLE IF NOT EXISTS ${PROJECT_PREFIX}_comments (
        id SERIAL PRIMARY KEY,
    ...
    `;
    await sql.unsafe(createCommentsTable);

    // 4. 创建项目特定评分表（带前缀）
    console.log(`⭐ 创建评分表 ${PROJECT_PREFIX}_ratings...`);
    const createRatingsTable = `
      CREATE TABLE IF NOT EXISTS ${PROJECT_PREFIX}_ratings (
        id SERIAL PRIMARY KEY,
        game_address_bar VARCHAR(100) NOT NULL,
    ...
    `;
    await sql.unsafe(createRatingsTable);

    // 5. 创建索引
    console.log('🔍 创建索引...');
    await sql.unsafe(`CREATE INDEX IF NOT EXISTS idx_${PROJECT_PREFIX}_comments_game_address_bar ON ${PROJECT_PREFIX}_comments(game_address_bar)`);
    await sql.unsafe(`CREATE INDEX IF NOT EXISTS idx_${PROJECT_PREFIX}_ratings_game_address_bar ON ${PROJECT_PREFIX}_ratings(game_address_bar)`);
    ...
    for (const game of sampleGames) {
      const existingGame = await sql`
        SELECT id FROM ${sql.unsafe(PROJECT_PREFIX + '_games')} WHERE address_bar = ${game.address_bar}
      `;
      
      if (existingGame.length === 0) {
        await sql`
          INSERT INTO ${sql.unsafe(PROJECT_PREFIX + '_games')} (address_bar, title)
